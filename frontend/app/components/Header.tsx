...
export default function Header() {
  ...
  return (
    <header
      className={`px-4 lg:px-[7rem] z-10 top-0 flex fixed mb-4 w-full justify-between items-center p-4 bg-[#093545] shadow-lg transition-all duration-300 ease-in-out ${
        isMenuOpen ? "h-[50vh]" : ""
      }`}
    >
      ...
      <div
        className={`flex lg:items-center lg:flex-row flex-col space-y-2 lg:justify-end lg:space-x-4 
          ${
            isMenuOpen
              ? "fixed inset-0 bg-[#092C39] p-4 transition-transform duration-300 ease-out transform translate-x-0"
      ...
      >
        <div
          onClick={() => closeMenu()}
          className="lg:hidden z-20 text-white fixed top-4 right-4 p-2 focus:outline-none transition-colors duration-300 ease-in-out transform hover:scale-105"
        >
          <button>✕</button>
        </div>
        ...
        <div className="flex gap-10 lg:flex-row flex-col">
          ...
          <div className="flex items-center space-y-2 lg:flex-row text-white lg:space-y-0 lg:space-x-4">
            <Link
              ...
              className="hover:cursor-pointer transition-colors flex gap-4 duration-300 ease-in-out transform hover:scale-105"
            >
              All Movies{" "}
              {allMovies ? (
                <CircularProgress size={24} color="inherit" />
